...
function canAccessConfig(member, cfg) {
  ...
  return Array.isArray(ids) && ids.length > 0 && ids.some(rid => member.roles.cache.has(rid));
}
...
function buildManagePlatformsModal(pageIndex = 0) {
  ...
  return {
    customId: pageIndex === 0 && totalPages <= 1 ? 'manage_platforms_modal' : `manage_platforms_modal_${pageIndex}`,
    title: `Manage platforms${pageLabel}`.slice(0, 45),
    components: [
      {
        type: 18, // ComponentType.Label
        label: 'Enable link conversion for these platforms',
        description: 'Checked = ON, unchecked = OFF. Use config again for more pages.',
        component: {
...
async function handleConfigButton(interaction) {
  ...
  if (id !== 'config_add_platform' && id !== 'config_update_platform' && id !== 'config_update_status' && id !== 'config_manage_platforms' && id !== 'config_remove_platform' && id !== 'config_scraper_image_limit') return false;

  const cfg = readConfig();
  if (!canAccessConfig(interaction.member, cfg)) {
    await interaction.reply({ content: 'You don\'t have permission to use config.', flags: MessageFlags.Ephemeral }).catch(() => {});
    return true;
  }
  ...
  try {
    if (id === 'config_add_platform') {
      ...
      modal.addComponents(
        ...
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            ...
            .setPlaceholder('e.g. (https?:\\/\\/(?:www\\.)?instagram\\.com\\/\\S+)')
            .setRequired(true)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
      ...
      if (platforms.length === 0) {
        await interaction.reply({ content: 'No platforms in config yet. Use "Add platform" first.', flags: MessageFlags.Ephemeral });
        return true;
      }
      const select = new StringSelectMenuBuilder()
        .setCustomId('config_select_platform_to_update')
        .setPlaceholder('Choose a platform to update…')
        .addOptions(platforms.slice(0, 25).map(key => ({
          label: key,
          value: key,
      ...
      await interaction.reply({
        content: '**Update platform** — Select which platform to edit. The next form will be pre-filled with current values.',
        components: [row],
        ...
        const select = new StringSelectMenuBuilder()
          ...
          .addOptions(
            Array.from({ length: totalPages }, (_, i) => {
              ...
              const pagePlatforms = platforms.slice(start, start + CHECKBOX_GROUP_MAX_OPTIONS);
      ...
      const platforms = Object.keys(cfg.platform || {});
      if (platforms.length === 0) {
        await interaction.reply({ content: 'No platforms in config yet. Nothing to remove.', flags: MessageFlags.Ephemeral });
        return true;
      }
      const select = new StringSelectMenuBuilder()
      ...
      const row = new ActionRowBuilder().addComponents(select);
      await interaction.reply({
        content: '**Remove platform** — Select which platform to remove. You\'ll get a confirmation step next.',
        components: [row],
        flags: MessageFlags.Ephemeral
      });
      ...
      const cfg = readConfig();
      const current = Math.min(10, Math.max(1, parseInt(cfg.ssScraperImageLimit, 10) || 10));
      const modal = new ModalBuilder()
    ...
    await interaction.reply({ content: 'Something went wrong opening the form.', flags: MessageFlags.Ephemeral }).catch(() => {});
  }
  return true;
}
...
async function handleConfigSelect(interaction) {
  if (interaction.customId === 'config_manage_platforms_page') {
    ...
    if (pageIndex < 0 || pageIndex >= totalPages) {
      await interaction.reply({ content: 'That page is no longer valid. Use **Manage platforms** again.', flags: MessageFlags.Ephemeral });
      return true;
    }
  ...
  if (interaction.customId === 'config_select_platform_to_remove') {
    ...
    if (!cfg.platform[platformKey]) {
      await interaction.reply({ content: 'That platform no longer exists in config.', flags: MessageFlags.Ephemeral });
      return true;
    }
    const modal = new ModalBuilder()
...
async function handleConfigModalSubmit(interaction, client) {
  ...
  if (id === 'platform_add_modal' && canAccess) {
    ...
    const pattern = interaction.fields.getTextInputValue('regex_pattern').trim();
    const conversion = interaction.fields.getTextInputValue('conversion_domain').trim();
    if (cfg.platform[name]) {
      await interaction.reply({ content: `Platform **${name}** already exists. Use "Update platform" to change it.`, flags: MessageFlags.Ephemeral });
      return true;
    }
  ...
  if (id.startsWith('platform_update_modal_') && canAccess) {
    ...
    const newProvider = interaction.fields.getTextInputValue('provider')?.trim();
    const newRegex = interaction.fields.getTextInputValue('regex')?.trim();
    if (!cfg.platform[platform]) {
      await interaction.reply({ content: `Platform **${platform}** no longer exists in config.`, flags: MessageFlags.Ephemeral });
      return true;
    }
  ...
  if ((id === 'manage_platforms_modal' || id.startsWith('manage_platforms_modal_')) && canAccess) {
    let enabledValues = [];
    try {
    ...
    const start = pageIndex * CHECKBOX_GROUP_MAX_OPTIONS;
    const keysInThisPage = platforms.slice(start, start + CHECKBOX_GROUP_MAX_OPTIONS);
    if (!cfg.enabled) cfg.enabled = {};
    for (const key of keysInThisPage) {
      cfg.enabled[key] = enabledValues.includes(key);
    }
    writeConfig(cfg);
    const totalEnabled = Object.keys(cfg.platform || {}).filter(k => cfg.enabled[k] !== false).length;
    const total = platforms.length;
    const totalPages = Math.ceil(total / CHECKBOX_GROUP_MAX_OPTIONS);
    const pageNote = totalPages > 1 ? ` (page ${pageIndex + 1}/${totalPages})` : '';
    await interaction.reply({
  ...
  if (id.startsWith('platform_remove_confirm||') && canAccess) {
    ...
    const typed = interaction.fields.getTextInputValue('remove_confirm_name').trim();
    ...
    if (!cfg.platform[platformKey]) {
      await interaction.reply({ content: `Platform **${platformKey}** no longer exists in config.`, flags: MessageFlags.Ephemeral });
      return true;
    }
  ...
  if (id === 'platform_add_modal' || id.startsWith('platform_update_modal_') || id === 'status_modal' || id === 'scraper_image_limit_modal' || id.startsWith('manage_platforms_modal') || id.startsWith('platform_remove_confirm||')) {
