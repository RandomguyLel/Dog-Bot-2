...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const channelName =
      interaction.channel?.name ?? (interaction.channel ? "DM" : "unknown");
    console.log(
      `[/config] ${interaction.user.tag} (${interaction.user.id}) in ${guildName} #${channelName}`,
    );
    ...
    const canAccess =
      isOwner ||
      (configRoleIds.length > 0 &&
        interaction.member?.roles.cache.hasAny(...configRoleIds));
    if (!canAccess) {
      await interaction.reply({
        content:
          "You don't have permission to use config. Only the bot owner or users with an allowed role can open this menu.",
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "Use the buttons below to change settings. Each opens a form you can fill and submit.\n\n" +
          ...
          "**Retry cooldown** — Delay between converter retry checks in milliseconds",
      );
    ...
    const row2 = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setDisabled(!canAccess),
      new ButtonBuilder()
        .setCustomId("config_retry_cooldown")
        .setLabel("Retry cooldown")
